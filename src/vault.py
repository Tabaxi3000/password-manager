...
def _atomic_write_bytes(path: Path, data: bytes) -> None:
    """
    Write `data` to `path` atomically, durably, and with mode 0600

    This is the write-temp / fsync / rename dance, extracted to module
    level so three callers can share it: the vault's own save(),
    `pv backup` (copying the encrypted file), and `pv export` (writing
    a plaintext dump). All three want the SAME guarantees — never a
    half-written file, never a world-readable window

      1. Open `path.tmp` with mode 0600 from the first syscall, so the
         file is never briefly visible to other local users
      2. write + fsync the bytes to physical disk
      3. os.replace the tmp onto the final path — atomic, so readers
         always see either the old complete file or the new one
      4. fsync the parent directory so the rename survives power loss
         (POSIX requirement; skipped on Windows, which lacks dir fsync)
    """
    path.parent.mkdir(parents = True, exist_ok = True)
    tmp_path = path.with_suffix(path.suffix + ".tmp")

    # Open with mode 0600 from the very first syscall. Using os.open +
    # os.write (instead of Path.write_bytes) lets us specify the mode
    # atomically; write_bytes would create the file with the process
    # umask (often 0644) and require a separate chmod, opening a brief
    ...
    try:
        try:
            os.write(fd, data)
            # fsync flushes the file's bytes from the kernel page cache
            # to the underlying disk. Without it the bytes are "written"
            # only in the sense that read() can see them — a power loss
            # at this moment loses everything
            os.fsync(fd)
        finally:
            os.close(fd)
        ...
        # If anything blew up between create and rename, clean up the
        # leftover tmp file so we do not litter the directory with junk
        # that confuses the next write
...
class Entry:
    ...
    # ISO 8601 timestamp of the last time this entry was READ via
    # `pv get`. Defaults to "" — an entry that has never been read
    # has no "last used" time, and inventing one would be a lie.
    # SECURITY TRADE-OFF: storing this means an attacker who steals
    # the (decrypted) vault learns which credential you touched most
    # recently. It stays inside the encrypted blob, so it only leaks
    # if the attacker already has the master password
    last_used_at: str = ""
    # Optional TOTP shared secret, stored as a base32 string exactly
    # as the website hands it to you. This is at LEAST as sensitive as
    # the password, which is why it belongs inside the encrypted vault
    # and never in a sidecar file. Empty string = no 2FA stored here
    totp_secret: str = ""
    ...
    def to_dict(self) -> dict[str, str]:
        ...
        """
        # asdict() walks the dataclass and produces a dict — every
        # field, including the ones added after v1, comes along for
        # free, which is exactly why we keep Entry a dataclass
        return asdict(self)
    ...
    def from_dict(cls, data: dict[str, Any]) -> Entry:
        ...
        return cls(
            ...
            updated_at = data.get("updated_at",
                                  ""),
            # Fields added after format v1. Old vaults predate them, so
            # `.get(..., "")` is what keeps those vaults openable — the
            # same missing-key handling created_at/updated_at use
...
class UnlockedVault:
    ...
    def _atomic_write(self, envelope_bytes: bytes) -> None:
        ...
        save() so the locking layer above stays readable. The actual
        write-temp-fsync-rename dance lives in the module-level
        `_atomic_write_bytes`, which `backup`/`export` reuse too
        """
        _atomic_write_bytes(self.path, envelope_bytes)
    ...
    def record_usage(self, name: str) -> Entry:
        """
        Stamp an entry's last_used_at with the current time and return it

        Called by `pv get` so the vault remembers when each credential
        was last looked at. Entry is frozen, so we cannot mutate it in
        place — we build a replacement with `dataclasses.replace`,
        which copies every other field unchanged and swaps only
        last_used_at. Crucially we do NOT touch updated_at: *reading*
        a credential is not *editing* it, and conflating the two would
        make every `get` look like a modification

        This only changes in-memory state. The caller must call save()
        to persist the new timestamp

        Raises EntryNotFoundError if there is no such entry
        """
        entry = self.get_entry(name)
        refreshed = replace(entry, last_used_at = _now_iso())
        self.entries[name] = refreshed
        return refreshed
    ...
    # -------------------------------------------------------------------------
    # KDF parameter upgrade — strengthen old vaults in place
    # -------------------------------------------------------------------------

    def needs_kdf_upgrade(self) -> bool:
        """
        Return True if this vault's KDF parameters are below current defaults

        Vaults remember the Argon2 parameters they were created with
        (that is the whole reason the file stores them). When we later
        raise the recommended defaults, old vaults keep opening with
        their original, now-weaker settings. This check spots that case

        We only flag a vault as needing an upgrade if it is WEAKER than
        the defaults in at least one dimension. A vault a user
        deliberately tuned STRONGER than the defaults is left alone —
        "upgrading" it would actually weaken it
        """
        current = self.kdf_parameters
        target = KdfParameters.defaults()
        return (
            current.time_cost < target.time_cost
            or current.memory_cost < target.memory_cost
            or current.parallelism < target.parallelism
        )

    def upgrade_kdf(self, master_password: str) -> None:
        """
        Re-derive the key under the current default Argon2 parameters

        Same password, fresh salt, stronger derivation. This reuses the
        master-password-rotation machinery (the work is identical: new
        salt, new key, re-encrypt on next save) but keeps the password
        unchanged. The caller passes the SAME master_password they just
        unlocked with

        Cost note: this pays the Argon2 derivation exactly ONCE — for
        the new key. We already hold the old key from unlock, so there
        is no second derivation of the old parameters. The user feels a
        single ~0.5-second pause, not two

        In-memory only; call save() afterward to persist the upgrade
        """
        # Passing the same password into the rotation path gives us an
        # in-place KDF upgrade for free — the salt and key rotate, the
        # secret does not
        self.change_master_password(
            master_password,
            kdf_parameters = KdfParameters.defaults(),
        )
...
# =============================================================================
# Vault-file validation — "is this a real vault?" without the password
# =============================================================================
# `restore` needs to confirm a backup file is a genuine, parseable
# vault BEFORE it overwrites the live one. We can do that without the
# master password — the envelope (version, kdf, cipher sections) is all
# public metadata. We never decrypt; we just prove the structure holds


def validate_vault_file(path: Path) -> None:
    """
    Raise VaultFormatError unless `path` is a structurally valid vault

    Reads the JSON envelope and runs it through the same `_parse_envelope`
    checks `unlock` uses — version, algorithm names, KDF parameter
    floors. Does NOT verify the password or decrypt anything; a file
    can be a valid vault you cannot open. The point is to refuse to
    overwrite a live vault with a backup that is obviously garbage
    ...
    VaultNotFoundError
        If no file exists at `path`
    VaultFormatError
        If the file is not parseable as a vault envelope
    """
    if not path.exists():
        raise VaultNotFoundError(f"No vault at {path}")
    try:
        ...
        raise VaultFormatError(
            f"File at {path} is not valid JSON: {exc}"
        ) from exc
    # _parse_envelope raises VaultFormatError on anything wrong; we
    # discard its return value because we only care that it succeeds
    _parse_envelope(envelope)


# =============================================================================
# Backup / restore — versioned snapshots of the encrypted file
# =============================================================================
# A backup is a byte-for-byte copy of the (already encrypted) vault
# file. It is exactly as safe to lose to disk forensics as the live
# vault — encrypted is encrypted. We reuse the advisory lock so a
# backup never copies a half-written vault, and the atomic writer so a
# crash mid-backup cannot leave a truncated snapshot


def list_backups(backup_dir: Path) -> list[Path]:
    """
    Return backup snapshot paths, newest first

    Filenames embed a sortable timestamp (vault-YYYY-MM-DD-HHMMSS.json),
    so a reverse alphabetical sort is also newest-first chronological.
    Returns an empty list if the directory does not exist
    """
    if not backup_dir.exists():
        return []
    return sorted(
        backup_dir.glob(BACKUP_FILENAME_GLOB),
        reverse = True,
    )
...
def create_backup(
    ...
    *,
    ...
    """
    Copy the encrypted vault into `backup_dir` and prune old snapshots

    Holds the vault's advisory lock for the whole read-copy-prune
    window, so we never snapshot a vault mid-save and never prune a
    file another `pv` process is mid-read on

    Parameters
    ----------
    vault_path
        The live vault to snapshot
    backup_dir
        Where snapshots live (created if missing)
    max_backups
        How many to keep. The newest `max_backups` survive; older ones
        are deleted. Must be >= 1

    Returns
    -------
    tuple[Path, list[Path]]
        (path of the new backup, list of pruned backup paths)

    Raises
    ------
    VaultNotFoundError
        If the live vault does not exist
    """
    if not vault_path.exists():
        raise VaultNotFoundError(f"No vault at {vault_path}")
    if max_backups < 1:
        raise ValueError("max_backups must be at least 1")

    backup_dir.mkdir(parents = True, exist_ok = True)

    with _file_lock(vault_path):
        # Read the encrypted bytes while holding the lock, so a
        # concurrent save() cannot swap the file out from under us
        data = vault_path.read_bytes()
        backup_name = datetime.now(UTC).strftime(BACKUP_FILENAME_FORMAT)
        backup_path = backup_dir / backup_name
        _atomic_write_bytes(backup_path, data)

        # Prune: keep the newest `max_backups`, delete the rest. The
        # list includes the snapshot we just wrote
...
def restore_backup(backup_path: Path, vault_path: Path) -> None:
    """
    Overwrite the live vault with a backup snapshot, atomically

    Validates that `backup_path` is a real vault envelope BEFORE
    touching the live file — restoring garbage over a working vault
    would be the worst possible outcome of a "restore" command. Then
    holds the vault lock and atomic-writes the backup bytes onto the
    live path, so a crash mid-restore leaves either the old or the new
    vault, never a corrupt blend
    ...
    VaultFormatError
        If `backup_path` is not a structurally valid vault
    """
    if not backup_path.exists():
        raise VaultNotFoundError(f"No backup at {backup_path}")
    # Refuse to clobber the live vault with something that is not a
    # vault. This is the whole reason restore is more than a `cp`
    validate_vault_file(backup_path)

    data = backup_path.read_bytes()
    with _file_lock(vault_path):
        _atomic_write_bytes(vault_path, data)


# =============================================================================
# Plaintext export / import — migration across the encrypted boundary
# =============================================================================
# DANGER: export writes every credential in CLEARTEXT. The CLI layer is
# responsible for the scary warnings and confirmation; this layer just
# does the serialization and the secure (0600, atomic) write. Import is
# the mirror image: parse, validate every row through Entry.from_dict
# (never trust the file structure), and hand back clean Entry objects


def export_entries(path: Path, entries: dict[str, Entry]) -> int:
    """
    Write every entry to `path` as a PLAINTEXT JSON file (mode 0600)

    The output is a small versioned envelope so a future importer can
    refuse a format it does not understand. Even though the contents
    are plaintext, we still write with mode 0600 and atomically — a
    world-readable secrets file is the exact foot-gun this whole
    project exists to avoid

    Returns the number of entries written
    """
    document = {
        EXPORT_KEY_VERSION: EXPORT_FORMAT_VERSION,
        EXPORT_KEY_ENTRIES: {
            name: entry.to_dict()
            for name, entry in entries.items()
        },
    }
    data = json.dumps(document, sort_keys = True, indent = 2).encode("utf-8")
    _atomic_write_bytes(path, data)
    return len(entries)


def read_export_file(path: Path) -> dict[str, Entry]:
    """
    Parse a file written by `export_entries` back into Entry objects

    Every row is validated through `Entry.from_dict`, exactly like
    `unlock` validates a decrypted vault — we do NOT trust the file's
    structure just because it claims to be ours. A single malformed
    row fails the whole import loudly rather than silently dropping it
    ...
    VaultFormatError
        If the file is not valid JSON, has the wrong version, or any
        entry row is malformed
    """
    if not path.exists():
        raise VaultNotFoundError(f"No import file at {path}")
    try:
        document = json.loads(path.read_text(encoding = "utf-8"))
    except (json.JSONDecodeError, UnicodeDecodeError) as exc:
        raise VaultFormatError(
            f"Import file at {path} is not valid JSON: {exc}"
        ) from exc

    if not isinstance(document, dict):
        raise VaultFormatError("Import file is not a JSON object")

    version = document.get(EXPORT_KEY_VERSION)
    if version != EXPORT_FORMAT_VERSION:
        raise VaultFormatError(
            f"Unsupported export version: {version} "
            f"(this build writes version {EXPORT_FORMAT_VERSION})"
        )

    raw_entries = document.get(EXPORT_KEY_ENTRIES)
    if not isinstance(raw_entries, dict):
        raise VaultFormatError("Import file missing 'entries' object")

    parsed: dict[str, Entry] = {}
    for name, row in raw_entries.items():
        if not isinstance(row, dict):
            raise VaultFormatError(
                f"Import entry '{name}' is not a JSON object"
            )
        # from_dict raises VaultFormatError on a bad row — let it
        parsed[name] = Entry.from_dict(row)
    return parsed
